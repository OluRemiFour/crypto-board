...
const PortfolioCard = () => {
  ...
  const { data: priceData, isLoading } = useQuery({
    queryKey: ['cryptoPrices', selectedCrypto.id],
    queryFn: () => fetchCryptoPrices(selectedCrypto.id),
  ...
  if (isLoading) {
    return (
      <div className="glass-card p-6 rounded-lg mb-8 animate-fade-in">
        <h2 className="text-xl font-semibold mb-6">{selectedCrypto.name} Performance</h2>
